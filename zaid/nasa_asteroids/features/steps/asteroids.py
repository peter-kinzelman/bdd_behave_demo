...
def step_impl(context, endpoint):
    global _url
    # I like this use of enums/constants for all URLS. We'll want to store those somewhere
...
def step_impl(context):
    ...
    for asteroid in _asteroids:
        ...
        diameterFromKMetarsToMeters = round(
            asteroid["estimated_diameter"]["kilometers"]["estimated_diameter_min"] * 1000, 5)
        diameterInMeters = round(asteroid["estimated_diameter"]["meters"]["estimated_diameter_min"],
                                 5)

        if diameterFromKMetarsToMeters != diameterInMeters:
            err.append('asteroid in index ' + str(
                counter) + ' dose not have the same distance in m and km')

        if type(asteroid["is_potentially_hazardous_asteroid"]) != bool:
            err.append('asteroid in index ' + str(
                counter) + ' dose not have "is_potentially_hazardous_asteroid" proparty')
    ...
    for asteroid in _asteroids:
        ...
        if (str(asteroid["absolute_magnitude_h"]) != str(absolute_magnitude_h)):
            err.append(['asteroid in the index of  ' + str(counter) +
                        ...
                        str(absolute_magnitude_h), str(asteroid["absolute_magnitude_h"])])

        SpkId = browser.find_element(By.XPATH, SPK_ID_SELECTOR).text
        if str(SpkId) != str(asteroid["id"]):
            err.append(['asteroid in the index of  ' + str(counter) +
                        "  in the api array dose not have the same id in UI", SpkId,
                        str(asteroid["id"])])
        ...
        if str(eccentricity) != str(asteroidFullData["orbital_data"]["eccentricity"]):
            err.append(['asteroid in the index of  ' + str(counter) +
                        ...
                        str(eccentricity), str(asteroidFullData["orbital_data"]["eccentricity"])])
        ...
        if str(inclination) != str(asteroidFullData["orbital_data"]["inclination"]):
            err.append(['asteroid in the index of  ' + str(counter) +
                        ...
                        str(inclination), str(asteroidFullData["orbital_data"]["inclination"])])
